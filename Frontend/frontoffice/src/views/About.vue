<template>
 ...
 <b-modal id="modal-1" ref="modal-1" hide-footer title="Questionnaire">
...
<b-modal id="modal-3" ref="modal-3"  hide-footer title="Confirmer Suppression">
    <p class="my-4">Supprimer Questionnaire</p>
  ...
  methods: {
    ...
    supprimer() {
      ...
      this.$refs['modal-3'].hide();
               this.isLoading = true;
     console.log("delted questionnaire " + this.iddelete);
        axios.delete("http://127.0.0.1:8000/questionnaire_detail/" + this.iddelete)
        .then(response2 => {
...
this.iddelete = data;
    ...
    axios.get("http://127.0.0.1:8000/questionnaire_detail/" + data)
        .then(response => {
            ...
            this.id = data;
            this.formupdate.dialect = response.data.dialect;
